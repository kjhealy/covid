...
us_states %>%
  ...
  geom_point(size = 3) + 
  scale_x_continuous(, trans = "log2", labels = scales::comma_format(accuracy = 1)) + 
...
us_states %>%
  ...
  ggplot(aes(x = date, y = cu_total, group = state, color = core, label = end_label)) + 
  geom_line(size = 0.5) + 
  geom_text_repel(segment.color = NA) + 
  scale_color_manual(values = c("gray70", "#E69F00", "#56B4E9", "#009E73")) + 
  scale_y_continuous(trans = "log2",
                     labels = scales::comma_format(accuracy = 1),
                     breaks = 2^c(seq(1, 18, 2))) +
  guides(color = FALSE) + 
  labs(title = "COVID-19 Cumulative Recorded Cases by US State",
       x = "Date", y = "Cumulative Reported Events (log 2 scale)", 
       caption = "Data: covidtracking.com / Graph: @kjhealy") +
        theme(plot.title = element_text(size = rel(2), face = "bold"),
          plot.subtitle = element_text(size = rel(1.5)),
          axis.text.y = element_text(size = rel(2)),
          axis.title.x = element_text(size = rel(1.5)),
